...
class PendulumLoss(PhysicsInformedLoss):
    ...
    def residual_error(self, x_input, y_pred) -> tf.Tensor:
        ...
        damping_coefficient = tf.cast(tf.multiply(self.scaler_std[1],x_input[:,1]) + self.scaler_mean[1],dtype=tf.float64)
    ...
    def compute_gradients(self, x_input):
        ...
        d2u_dt2 = outer_tape.gradient(du_dt, x_input)
        d2u_dt2 = tf.divide(d2u_dt2, tf.square(self.scaler_std))
        return u[:,0], tf.cast(du_dt[:,0],dtype=tf.float64), tf.cast(d2u_dt2[:,0],dtype=tf.float64)
    ...
    def initial_condition_error(self, x_input) -> tf.Tensor:
        ...
        residual_position = u - tf.constant(self.initial_position,dtype=tf.float64)
        residual_velocity = du_dt - tf.constant(self.initial_velocity,dtype=tf.float64)
        return tf.add(tf.reduce_sum(tf.square(residual_velocity)),tf.reduce_sum(tf.square(residual_position)))