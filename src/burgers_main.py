...
from src.model.burgers_nerual_network import BurgersNerualNetwork
from src.loss.burgers_loss import BurgersLoss
...
if __name__ == "__main__":
    data_path = os.path.join("..","data","BURGERS.mat")
    burger_data = loadmat(data_path)
    u = burger_data["u_solutions"]
    t = burger_data["T"].flatten()[:,None]
    x = burger_data["x"].flatten()[:,None]
    ...
    ## Loss and Optimizer
    loss_function = BurgersLoss(burgers_parameters)
    ...
    ## Dataloader
    dataloader = DataLoaderFactory.create_data_loader("burgers",test_size=0.2,random_state=42)

    ## Build NN
    NN = BurgersNerualNetwork(model_architecture,burgers_parameters=burgers_parameters)
